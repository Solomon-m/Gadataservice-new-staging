...
var matcherFlag="";
		...
		console.debug("Inside respond war");
...
//	self.postMessage(e.data);
	console.debug("Checking in sent request to respond js::"+e.target);
	var reqObj=e.data;
		w_Data=reqObj.data;
		matcherFlag=reqObj.matcher;
	
	if(reqObj.report=="sendDetails")
		{  console.debug("Inside respond js addEventListener in war:::");
...
{//console.log(index);
	console.debug("Inside respond getUniqueDimensionFromWorker w::");
	var i,
    len=rowArray.length,
    out=[],
    obj={};
	...
	function agentActionSummary()
	  {
		console.debug("Inside respond agentActionSummay w::");
...
{var i,j=0;
   console.debug("Inside respond filterDataByDimension w::");
...
var patt = new RegExp(dimension.toLowerCase());
	...
	//console.log("Checking filteredData::s"+filteredData);
...
{
	  console.debug("Inside respond getCallDetails war::");
...
{     console.debug("Inside respond filteDimension w::");
