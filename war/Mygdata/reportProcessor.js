...
{ 
	...
	if(page=="Send"){
		 console.debug("Selected Send in reportProcessor in Mygdata:::");
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'Done',1,'false',"true"));
		setActiveTab("done");

		}
		if(page=="Save"){
			 console.debug("Selected Save in reportProcessor in Mygdata:::");
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'Save',1,'false',"true"));
		setActiveTab("save");

		}
	if(page=="Chat"){
		 console.debug("Selected Chat in reportProcessor in Mygdata:::");
		...
		if(page=="AR"){
			 console.debug("Selected AR in reportProcessor in Mygdata:::");
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'eventToTalk',1,'false',"true"));
		setActiveTab("eventToTalk");

		}	
		if(page=="Outbound")
		{ console.debug("Selected Outbound in reportProcessor in Mygdata:::");
		...
		{ 
	          console.debug("Selected CX Feedback Data in reportProcessor in Mygdata:::");
	...
	if(page=="SB Notes")
	{
        console.debug("Selected SB Notes in reportProcessor in Mygdata:::");
...
{	console.debug("Inside reportProcessor Mygdata processGAData::");
...
{	console.debug("Inside reportProcessor Mygdata getEventDescriptionForCategory::");
	var rowObj;
	var uniqueDimension;
	var filteredData;
...
{var i,j=0;
console.debug("Inside reportProcessor Mygdata filterDataByDimension::");
...
{//console.log(index);
	console.debug("Inside reportProcessor Mygdata getUniqueDimensionFromGAData::");
	var i,
    len=rowArray.length,
    out=[],
    obj={};
...
function isObjNull(obj)
{
  ...
  {
	  console.debug("Inside reportProcessor Mygdata getCallDetails::"); 
  ...
  {  console.debug("Inside reportProcessor Mygdata createCallerMap::");
  ...
  {		console.debug("Inside reportProcessor Mygdata getCallerAccountDetails::");
	  createCallerMap(3);
	  var callerAccount=[];
  ...
  {   console.debug("Inside reportProcessor Mygdata filterDimension::");
  ...
  {   console.debug("Inside reportProcessor Mygdata getCallerHistoryDetails::");
	  return mapToArray(filterDataByDimension(jsonData,"Account Load",1));
	  	    	  
  }
  console.log("filter event data");
  function filterEventData()
  {  console.debug("Inside reportProcessor Mygdata filterEventData::");
	  var dimension=document.getElementById("dimension").value;
	  var filterData=document.getElementById("filterData").value;
  ...
  { console.debug("Inside reportProcessor Mygdata getDataSet::");
...
console.log(dataset);
  ...
  {var callerAccount=[];
  console.debug("Inside reportProcessor Mygdata filterCallDataMap::");
  ...
  {  console.debug("Inside reportProcessor Mygdata groupDataByDimension::");
  ...
  {   console.debug("Inside reportProcessor Mygdata callActionHandler::");
  ...
  function pivotDataByDimension() {
	  console.debug("Inside reportProcessor Mygdata pivotDataDimension::");
	  var options = {'showRowNumber': true};
  ...
  {    console.debug("Inside reportProcessor Mygdata DownloadJSON2CSV::");
 	...
 	function agentActionSummary()
 	  {   console.debug("Inside reportProcessor Mygdata agentActionSummary::");
 	      console.debug(":****agentActionSummary****:");
...
// 		}
 	...
 	{
 		console.debug("Inside reportProcessor Mygdata drawAgentDetails::");
//  		columnData=agentActionSummary();
  ...
  table.draw(dataView1, options);
 	...
 	{   console.debug("Inside reportProcessor Mygdata getCXFeedbackReport::");
 	...
 	{	console.debug("Inside reportProcessor Mygdata createAgentDetailsCSVReport::");
 		csvData= null;
 	...
 	{   console.debug("Inside reportProcessor Mygdata renameAction::");
...
{   console.debug("Inside reportProcessor Mygdata CallConclusionReport::");
...
function drawCCDetail()
	{
	console.debug("Inside reportProcessor Mygdata drawCCDetail::");
